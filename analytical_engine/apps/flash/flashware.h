...
#ifndef ANALYTICAL_ENGINE_APPS_FLASH_APP_FLASHWARE_H_
#define ANALYTICAL_ENGINE_APPS_FLASH_APP_FLASHWARE_H_
...
class FlashWare : public grape::Communicator, public grape::ParallelEngine {
 ...
 public:
  ...
  void GetActiveVerticesAndSetStates(std::vector<vid_t>& result, grape::Bitset& d);
  void SyncBitset(grape::Bitset& tmp, grape::Bitset& d);
  void SyncBitset(grape::Bitset& b);
 ...
 private:
  ...
  bool sync_all_;
  grape::Bitset nb_ids_;

  value_t* states_;
  value_t* next_states_;
  grape::Bitset is_active_;
  int step_;
...
void FlashWare<fragment_t, value_t>::InitFlashWare(const grape::CommSpec& comm_spec,
                                                   const bool& sync_all,
                                                   std::shared_ptr<fragment_t> graph) {
...
template <typename fragment_t, class value_t>
void FlashWare<fragment_t, value_t>::GetActiveVerticesAndSetStates(
    std::vector<vid_t>& result, grape::Bitset& d) {
  SyncBitset(is_active_, d);
  GetActiveVerticesAndSetStates(result);
}

template <typename fragment_t, class value_t>
void FlashWare<fragment_t, value_t>::SyncBitset(grape::Bitset& tmp, grape::Bitset& res) {
...
template <typename fragment_t, class value_t>
void FlashWare<fragment_t, value_t>::SyncBitset(grape::Bitset& b) {
...
#endif  // ANALYTICAL_ENGINE_APPS_FLASH_APP_FLASHWARE_H_
